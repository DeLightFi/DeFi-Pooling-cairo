export function Compute(tokens, selected_data) {
    ...
    const pnl_evolution_num = (((last_pnl_dollars_total - first_pnl_dollars_total) / Math.abs(first_pnl_dollars_total)) * 100);

    if (!isFinite(pnl_evolution_num)) {
        pnl_evolution = "100";
    } else {
        pnl_evolution = pnl_evolution_num.toFixed(2)
        if (pnl_evolution[0] !== "-") {
